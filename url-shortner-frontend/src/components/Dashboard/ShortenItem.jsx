...
const ShortenItem = ({ originalUrl, shortUrl, clickCount, createdDate }) => {
  ...
  const fetchMyShortUrl = async () => {
    setLoader(true);
    try{
        const { data } = await api.get(`/api/urls/analytics/${selectedUrl}?startDate=2025-06-20T00:00:00&endDate=2025-12-30T23:59:59`, {
                        headers: {
    ...
    <div className="bg-slate-100 shadow-lg border border-dotted border-slate-500 px-6 sm:py-1 py-3 rounded-md transition-all duration-100">
      <div className="flex sm:flex-row flex-col sm:justify-between w-full sm:gap-0 gap-5 py-5">
        <div className="flex-1 sm:space-y-1 max-w-full overflow-x-auto overflow-y-hidden">
          <div className="text-slate-900 pb-1 sm:pb-0 flex items-center gap-2">
            <a
              ...
              className="text-[17px] font-sans font-[600] text-[#2a5bd7]"
            >
              {`${subDomain}/${shortUrl}`}
            </a>
            <FaExternalLinkAlt className="text-[#2a5bd7]" />
          </div>
        ...
        <div className="flex flex-1 sm:justify-end items-center gap-4">
          <div
            onClick={handleCopy}
            style={{ backgroundColor: "#3364F7" }}
            className="flex cursor-pointer gap-1 items-center py-2 font-semibold shadow-md shadow-slate-500 px-6 rounded-md text-white"
          >
